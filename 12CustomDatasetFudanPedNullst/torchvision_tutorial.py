...
`torch.utils.PennFudanPed.Dataset`{.interpreted-text role="class"} class, and
...
``` {.sourceCode .python}
wget https://www.cis.upenn.edu/~jshi/ped_html/PennFudanPed.zip -P PennFudanPed
cd PennFudanPed && unzip PennFudanPed.zip
```
...
`torch.utils.PennFudanPed.Dataset`{.interpreted-text role="class"} class for
...
class PennFudanDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        boxes = masks_to_boxes(masks) #tensor([[114.,  47., 243., 331.]]) Tensor (1, 4)
...
PennFudanPed augmentation pipelines for Object Detection and Segmentation tasks.

Let's write some helper functions for PennFudanPed augmentation /
transformation:
...
expects during training and inference time on sample PennFudanPed.
...
wrote a `torch.utils.PennFudanPed.Dataset`{.interpreted-text role="class"} class
...
training, check `references/detection/testing_the_prototype.py`, which is present in the
