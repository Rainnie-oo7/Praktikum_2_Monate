...
def extract_polygons(path2):
    imgsizepath = osp.normpath(osp.join(osp.dirname(__file__), "data/CARDS_COURTYARD_B_T/frame_0011.jpg"))
...
def show_black_img(mymasks):
    plt.imshow(mymasks[0])
    plt.show()

if __name__ == '__main__':
    path2 = osp.normpath((osp.join(osp.dirname(__file__), "data/CARDS_COURTYARD_B_T/")))
    mymasks = extract_polygons(path2)
    show_black_img(mymasks)