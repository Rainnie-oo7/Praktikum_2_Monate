...
from tensorflow.python.ops.gen_linalg_ops import batch_matrix_solve_ls
...
class Mydataset(Dataset):
    ...
    def __init__(self, path, transform=None):
        ...
        if not isinstance(self.path, (str, bytes, os.PathLike)):
            raise TypeError(f"Ungültiger Pfadtyp für `path`: {type(self.path)}")

            # Durchlaufe das Verzeichnis und überprüfe `dir`
        for dir in os.listdir(self.path):
            if not isinstance(dir, (str, bytes, os.PathLike)):
                raise TypeError(f"Ungültiger Typ für `dir`: {type(dir)}")
            self.full_dir_path = os.path.join(self.path, dir)
            print(self.full_dir_path)  # Beispiel: Ausgabe des vollständigen Pfads
    ...
    def gcornersmkbb(self):
        ...
        #     print("boxes", boxes)
        # else:
        #     print(f"boxes hat zu wenige/viele Ecken {bbml}, {bbmr}, {bbyl}, {bbyr} und Länge {len(bbml)}, {len(bbmr)}, {len(bbyl)}, {len(bbyr)}")
        # for i in range(len(labels)):
        #     boxes_labels_data['labels']
            # boxes_labels_data.append({'labels': labels[i], 'boxes': boxes[i]})
        print("boxes_labels_data", boxes_labels_data)
        print("boxes_labels_datadim", boxes_labels_data.dim())
        # print("boxes_labels_datashape", boxes_labels_data.shape)
        print("boxes_labels_datatype", type(boxes_labels_data))
