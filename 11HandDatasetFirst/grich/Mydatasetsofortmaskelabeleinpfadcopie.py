...
class Mydataset(Dataset):
                ...
                for file in os.listdir(self.full_dir_path):
                    ...
                    if file.startswith('frame'):
                        ...
                        if file.startswith('polygons'):
                            ...
                            # print("hallooo")
                            self.mymasks = self.mymasks + self.extract_polygons(self.full_path)
    ...
    def extract_polygons(self, full_path):
        ...
        for idx, (label, array_mlmrylyr) in enumerate(zip(labels, maske)):
            ...
            for mask in array_mlmrylyr:                     # Hier checken ob was drin ist, aber sind nicht-WErte nicht super schnell durchgefegt ?
                print("das ist mask:", mask)
                print("das ist shapemask1:", mask.shape)
                ...
                if mask.size > 0:
                    ...
                    #     return matrix
                    boxess = masks_to_boxes(mask)
                    x0 = boxess[:, 0]
                    y0 = boxess[:, 1]
                    x1 = boxess[:, 2]
                    y1 = boxess[:, 3]
                    # Erstelle den Boxen-Tensor (N, 4) für (x0, y0, x1, y1)
                    ...
                    print("das ist typeboxes_tensor:", type(boxes_tensor))
        ...
        # print("maske:", maske)
        # print(type(maske))
        # print("mymasks0:", mymasks)
                print("self.mymasksexpoly:", len(self.mymasks))

        return mymasks, boxes_tensor
    ...
    # def getBoxesCoordnts(self):
    #     self.objectid = self.comparemasktoimg()
    #     boxess = masks_to_boxes(self.objectid)
    #     x0 = boxess[:, 0]
    #     y0 = boxess[:, 1]
    #     x1 = boxess[:, 2]
    #     y1 = boxess[:, 3]
    ...
    def __getitem__(self, idx):
        ...
        # mymasks = self.extract_polygons(self.full_dir_path)
        ...
        print(f"Type of objectid: {type(champ)}")
        print(f"shape of objectid:", champ.shape)

        boxes = self.getBoxesCoordnts()
        print(f"Type of boxes: {type(boxes)}")
        print(f"Content of boxes: {boxes}")
        print("das ist boxes:", boxes)
        ...
        target = {}
        # target["boxes"] = tv_tensors.BoundingBoxes(boxes, format="XYXY", canvas_size=(1280,720))
        target["boxes"] = boxes
        print("das ist boxes:", boxes)

        # target["boxes"] = tv_tensors.BoundingBoxes(boxes, format="XYXY", canvas_size=F.get_size(img))
        print("das ist target[.boxes].:", target["boxes"])
        print("das ist typetarget[.boxes].:", type(target["boxes"]))
