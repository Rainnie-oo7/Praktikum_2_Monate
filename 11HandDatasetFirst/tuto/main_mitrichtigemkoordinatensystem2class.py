...
class Coordinatesdraw:
    def __init__(self, parent_folder):
        ...
        #     for filename in fnmatch.filter(filenames, 'polygons*'):
        #         self.mat_data = scipy.io.loadmat(filename)
        #         self.PennFudanPed = self.mat_data['polygons']
        #         print(self.mat_data.keys())
        #
        #         for i in range(len(self.PennFudanPed)):
        #             dir_paths = []
        #             image_paths = []
        #             for filename, dirnames, filenames in os.walk(parent_folder):
        #                 dir_paths.append(dirnames)
        #                 # dir_paths.sort()
        #             for i in range(len(dir_paths)):
        #                 print(f"Processing folder: {dir_paths[i]}")
        #             for filename, dirnames, filenames in os.walk(parent_folder):
        #                 for i in image_paths:
        #                     if i.startswith('frame'):
        #                         self.img = Image.open(file_paths[i])
        ...
        self.run()
    # def iter_frame_files(self, parent_folder):
...
# Example usage:
coordinatesdraw = Coordinatesdraw('/home/boris.grillborzer/PycharmProjects/00CNNundRNN/boris.grillborzer/11HandDatasetFirst/PennFudanPed/CARDS_COURTYARD_B_T/')
