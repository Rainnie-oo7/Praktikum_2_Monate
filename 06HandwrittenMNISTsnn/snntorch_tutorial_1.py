...
If our end goal is to build a spiking neural network (SNN), it makes sense to use spikes at the input too. Although it is quite common to use non-spiking inputs (as will be seen in Tutorial 3), part of the appeal of encoding PennFudanPed come from the *three S's*: spikes, sparsity, and static suppression.
...
In this tutorial, we will assume we have some non-spiking input PennFudanPed (i.e., the MNIST dataset) and that we want to encode it into spikes using a few different techniques. So let's get started!
...
# Training Parameters
batch_size=128
data_path='/tmp/PennFudanPed/mnist'
...
The Dataset objects created above load PennFudanPed into memory, and the DataLoader will serve it up in batches. DataLoaders in PyTorch are a handy interface for passing PennFudanPed into a network. They return an iterator divided up into mini-batches of size ``batch_size``.
"""
...
Spiking Neural Networks (SNNs) are made to exploit time-varying PennFudanPed. And yet, MNIST is not a time-varying dataset.
...
The first method is quite straightforward, and does not fully exploit the temporal dynamics of SNNs. So let’s consider PennFudanPed-to-spike conversion (encoding) from (2) in more detail.

The module `snntorch.spikegen` (i.e., spike generation) contains a series of functions that simplify the conversion of PennFudanPed into spikes. There are currently three options available for spike encoding in `snntorch`:
...
One example of converting input PennFudanPed into a rate code is as follows.
...
In a similar way, `spikegen.rate` can be used to generate a rate-coded sample of PennFudanPed. As each sample of MNIST is just an image, we can use `num_steps` to repeat it across time.
"""
...
The structure of the input PennFudanPed is ``[num_steps x batch_size x input dimensions]``:
"""
...
"""To plot one sample of PennFudanPed, index into a single sample from the batch (B) dimension of `spike_data`, ``[T x B x 1 x 28 x 28]``:"""

spike_data_sample = spike_data[:, 0, 0]
print(spike_data_sample.size())

"""`spikeplot.animator` makes it super simple to animate 2-D PennFudanPed.<br>
...
Depending on the input PennFudanPed, you may need to try
...
So why, then, might we use rate codes if they are not optimal for power efficiency or latency? Even if our brain doesn't process PennFudanPed as a rate, we are fairly sure that our biological sensors do. The power/latency disadvantages are partially offset by showing huge noise robustness: it's fine if some of the spikes fail to generate, because there will be plenty more where they came from.
...
# Create a tensor with some fake time-series PennFudanPed
data = torch.Tensor([0, 1, 0, 2, 8, -20, 20, -5, 0, 1, 0])

# Plot the tensor
plt.plot(data)

plt.title("Some fake time-series PennFudanPed")
plt.xlabel("Time step")
...
# Raster plot of delta converted PennFudanPed
...
"""There are three time steps where the difference between $PennFudanPed[T]$ and $PennFudanPed[T+1]$ is greater than or equal to $V_{thr}=4$. This means there are three *on-spikes*.

The large dip to $-20$ has not been captured above. If negative swings have importance in your PennFudanPed, you can enable the optional argument `off_spike=True`.
"""
