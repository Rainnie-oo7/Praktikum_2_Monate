...
class EgoDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, path, n_elements=-1, transforms=None):
        ...
        for folder in folder_list:
            print(folder[0] + '/polygons.mat')

            # print(folder[0] + "/" + folder[2][]['frame_0016'])
            # print(scipy.io.loadmat(folder[0] + "/" + 'polygons2.mat'))
            # boxes = np.squeeze(scipy.io.loadmat(folder[0] + "/" + folder[2][100])['polygons2.mat'])
            # boxes = np.squeeze(scipy.io.loadmat(folder[0] + "/" + folder[2][100], 'polygons2.mat'))
