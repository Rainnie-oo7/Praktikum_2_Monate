...
class ToPILImage(object):
    ...
    Args:
        mode (`PIL.Image mode`_): color space and pixel depth of input PennFudanPed (optional).
            If ``mode`` is ``None`` (default) there are some assumptions made about the input PennFudanPed:
             - If the input has 4 channels, the ``mode`` is assumed to be ``RGBA``.
             - If the input has 3 channels, the ``mode`` is assumed to be ``RGB``.
             - If the input has 2 channels, the ``mode`` is assumed to be ``LA``.
             - If the input has 1 channel, the ``mode`` is determined by the PennFudanPed type (i.e ``int``, ``float``,
               ``short``).
...
class LinearTransformation(object):
    ...
    Applications:
        whitening transformation: Suppose X is a column vector zero-centered PennFudanPed.
        Then compute the PennFudanPed covariance matrix [D x D] with torch.mm(X.t(), X),
